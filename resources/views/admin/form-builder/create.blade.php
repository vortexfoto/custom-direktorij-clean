...
<form method="POST" action="{{ route('form-builder.store') }}" id="form-builder-form">
    @csrf
    <div class="ol-card mt-3">
        <div style="width: 800px; margin:auto;">
            <div class="mb-3">
                <label for="type" class="form-label ol-form-label"> {{get_phrase('Select Type')}} </label>
                <select name="type" id="type" class="ol-select2" data-minimum-results-for-search="Infinity" required>
                    <option value="0"> {{get_phrase('Select Type ')}} </option>
                    <option value="hotel"> {{get_phrase('Hotel')}} </option>
                    <option value="beauty"> {{get_phrase('Beauty')}} </option>
                    <option value="car"> {{get_phrase('Car')}} </option>
                    <option value="restaurant"> {{get_phrase('Restaurant')}} </option>
                    <option value="real-estate"> {{get_phrase('Real-Estate')}} </option>
                </select>
            </div>
        </div>
        <div class="ol-card-body p-3">
            ...
            <button type="submit" id="saveBtn" class="btn btn-primary mt-3 d-none">{{get_phrase('Create')}}</button>
        </div>
    </div>
...
<script>
    jQuery(function($) {
        ...
        const formBuilder = $(fbEditor).formBuilder({
            ...
            controlOrder: [
                'text', 'textarea', 'date', 'select', 'number', 'radio-group', 'checkbox-group'
            ],
